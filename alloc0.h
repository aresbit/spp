...
struct Scalar_Adaptor {
    ...
    [[nodiscard]] static T* make(Args&&... args) noexcept {
        ...
        new(mem) T{spp::forward<Args>(args)...};
        return mem;
    }
...
#define SPP_REGION2(COUNTER) __region_##COUNTER
#define SPP_REGION1(R, BRAND, COUNTER)                                                             \
    if constexpr(constexpr u64 R = BRAND)                                                          \
        if(::spp::Region_Allocator::Scope<R> SPP_REGION2(COUNTER){})

#define Region(R) SPP_REGION1(R, SPP_LOCATION_HASH, __COUNTER__)
...
struct Free_List {
    ...
    [[nodiscard]] T* make(Args&&... args) noexcept {
        return new(alloc()) T{spp::forward<Args>(args)...};
    }
