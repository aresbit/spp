...
template<typename T>
SPP_MSVC_INTRINSIC SPP_FORCE_INLINE constexpr typename Remove_Reference<T>::type&&
move(T&& value) noexcept {
    return static_cast<typename Remove_Reference<T>::type&&>(value);
}

template<typename T>
SPP_MSVC_INTRINSIC SPP_FORCE_INLINE constexpr T&&
...
struct Is_Invocable {
    constexpr static bool value = requires(F&& f, Args&&... args) {
        { spp::detail::forward<F>(f)(spp::detail::forward<Args>(args)...) };
    };
};
...
template<class T>
[[nodiscard]] SPP_FORCE_INLINE constexpr T* launder(T* p) noexcept
