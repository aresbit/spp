...
SPP_RECORD(Vec2, SPP_FIELD(x), SPP_FIELD(y));
SPP_RECORD(Vec3, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z));
SPP_RECORD(Vec4, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z), SPP_FIELD(w));
template<u64 N>
SPP_TEMPLATE_RECORD(VecN, N, SPP_FIELD(data));

SPP_RECORD(Vec2i, SPP_FIELD(x), SPP_FIELD(y));
SPP_RECORD(Vec3i, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z));
SPP_RECORD(Vec4i, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z), SPP_FIELD(w));
template<u64 N>
SPP_TEMPLATE_RECORD(VecNi, N, SPP_FIELD(data));

SPP_RECORD(Vec2u, SPP_FIELD(x), SPP_FIELD(y));
SPP_RECORD(Vec3u, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z));
SPP_RECORD(Vec4u, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z), SPP_FIELD(w));
template<u64 N>
SPP_TEMPLATE_RECORD(VecNu, N, SPP_FIELD(data));

SPP_RECORD(Mat4, SPP_FIELD(columns));
SPP_RECORD(BBox, SPP_FIELD(min), SPP_FIELD(max));
SPP_RECORD(Quat, SPP_FIELD(x), SPP_FIELD(y), SPP_FIELD(z), SPP_FIELD(w));
...
struct Hash<Math::Vect<T, N>> {
    constexpr static u64 hash(const Math::Vect<T, N>& v) noexcept {
        u64 h = 0;
        for(u64 i = 0; i < N; i++) h = hash_combine(h, spp::hash(v[i]));
        return h;
    }
};
