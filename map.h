...
struct Map_Slot {
    ...
    constexpr explicit Map_Slot(K&& key, V&& value) noexcept : hash(hash_nonzero(key)) {
        data.construct(spp::move(key), spp::move(value));
    }
...
struct Map {
    ...
    explicit Map(Ss&&... init) noexcept {
        (insert(spp::move(init.first), spp::move(init.second)), ...);
    }
    ...
    void reserve(u64 new_capacity) noexcept {
        ...
        for(u64 i = 0; i < old_capacity; i++) {
            if(old_data[i].hash != Slot::EMPTY)
                static_cast<void>(insert_slot(spp::move(old_data[i])));
        }
        A::free(old_data);
    }
    ...
    {
        return insert(spp::move(key), V{value});
    }

    V& insert(const K& key, V&& value) noexcept
        requires Copy_Constructable<K>
    {
        return insert(K{key}, spp::move(value));
    }

    V& insert(K&& key, V&& value) noexcept {
        if(full()) grow();
        Slot slot{spp::move(key), spp::move(value)};
        Slot& placed = insert_slot(spp::move(slot));
        length_ += 1;
    ...
    V& emplace(K&& key, Args&&... args) noexcept {
        if(full()) grow();
        Slot slot{spp::move(key), V{spp::forward<Args>(args)...}};
        Slot& placed = insert_slot(spp::move(slot));
        length_ += 1;
        return placed.data->second;
    }
...
template<Key K, Move_Constructable V>
SPP_NAMED_TEMPLATE_RECORD(::spp::detail::Map_Slot, "Slot", SPP_PACK(K, V), SPP_FIELD(hash),
                          SPP_FIELD(data_));

template<Key K, Move_Constructable V, Allocator A>
SPP_TEMPLATE_RECORD(Map, SPP_PACK(K, V, A), SPP_FIELD(data_), SPP_FIELD(capacity_),
                    SPP_FIELD(length_), SPP_FIELD(usable_), SPP_FIELD(shift_));
