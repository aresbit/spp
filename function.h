...
struct Function<Words, R(Args...)> {
    ...
    [[nodiscard]] R operator()(Args... args) noexcept {
        assert(vtable);
        return invoke(spp::forward<Args>(args)...);
    }
...
private:
    ...
    [[nodiscard]] R invoke(Args... args) noexcept {
        ...
        return do_invoke(storage, spp::forward<Args>(args)...);
    }
    ...
    static void f_move(F* dst, F* src) noexcept {
        if constexpr(Trivially_Movable<F>) {
            ...
            new(dst) F{spp::move(*src)};
        }
    }
    template<typename F>
    [[nodiscard]] static R f_invoke(F* src, Args... args) noexcept {
        return src->operator()(spp::forward<Args>(args)...);
    }
...
template<typename F>
SPP_TEMPLATE_RECORD(Function, F);

template<u64 N, typename F>
SPP_TEMPLATE_RECORD(FunctionN, SPP_PACK(N, F));
