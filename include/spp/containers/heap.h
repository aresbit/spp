...
struct Heap {
    ...
    {
        if(full()) grow();
        new(&data_[length_++]) T{spp::move(value)};
        reheap_up(length_ - 1);
    }
    ...
    {
        if(full()) grow();
        new(&data_[length_++]) T{spp::forward<Args>(args)...};
        reheap_up(length_ - 1);
    }
        ...
        requires Trivially_Movable<T> || Move_Constructable<T>
    {
        ...
        if(length_ > 0) {
            if constexpr(Trivially_Movable<T>) {
                ...
                static_assert(Move_Constructable<T>);
                new(data_) T{spp::move(data_[length_])};
            }
            reheap_down(0);
        }
    }
...
private:
    void swap(u64 a, u64 b) noexcept
        requires Move_Constructable<T>
    {
        T temp{spp::move(data_[a])};
        new(&data_[a]) T{spp::move(data_[b])};
        new(&data_[b]) T{spp::move(temp)};
    }
...
SPP_TEMPLATE_RECORD(Heap, SPP_PACK(T, A), SPP_FIELD(data_), SPP_FIELD(length_),
                    SPP_FIELD(capacity_));
