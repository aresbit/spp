...
struct Stack {
    ...
    explicit Stack(S&&... init) noexcept : data_(spp::forward<S>(init)...) {
    }
    ...
    {
        return data_.push(spp::move(value));
    }

    template<typename... Args>
        requires Constructable<T, Args...>
    T& emplace(Args&&... args) noexcept {
        return data_.emplace(spp::forward<Args>(args)...);
    }
...
SPP_TEMPLATE_RECORD(Stack, SPP_PACK(T, A), SPP_FIELD(data_));
