...
struct Queue {
        ...
        if constexpr(Trivially_Movable<T>) {
            ...
            if(length_ <= last_) {
                for(u64 i = 0; i < length_; i++) {
                    new(&new_data[i]) T{spp::move(start[i])};
                }
            } else {
                u64 first = length_ - last_;
                for(u64 i = 0; i < first; i++) {
                    new(&new_data[i]) T{spp::move(start[i + capacity_])};
                }
        ...
        requires Move_Constructable<T>
    {
        ...
        new(&data_[last_]) T{spp::move(value)};
        T& ret = data_[last_];
        ...
        requires Constructable<T, Args...>
    {
        ...
        new(&data_[last_]) T{spp::forward<Args>(args)...};
        T& ret = data_[last_];
        ...
        while(consumed < length_) {
            ...
            if(found) {
                if constexpr(Trivially_Movable<T>) {
                    ...
                    static_assert(Move_Constructable<T>);
                    new(&data_[next]) T{spp::move(data_[idx])};
                }
...
SPP_TEMPLATE_RECORD(Queue, SPP_PACK(T, A), SPP_FIELD(data_), SPP_FIELD(length_), SPP_FIELD(last_),
                    SPP_FIELD(capacity_));
