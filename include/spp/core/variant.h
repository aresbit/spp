...
struct Variant {
        ...
        requires(Move_Constructable<Ts> && ...)
    {
        if constexpr((Trivially_Movable<Ts> && ...)) {
            ...
            src.match(Overload{[this](Ts& v) { this->construct(spp::move(v)); }...});
        };
    }
    ...
    [[nodiscard]] auto match(F&& f) & noexcept {
        return Accessors<With_Lvalue_Ref, false>::apply(spp::forward<F>(f), data_, index_);
    }
    template<typename F>
        requires((Invocable<F, Ts &&> && ...) && All_Same<Invoke_Result<F, Ts &&>...>)
    [[nodiscard]] auto match(F&& f) && noexcept {
        return Accessors<With_Rvalue_Ref, false>::apply(spp::forward<F>(f), data_, index_);
    }
    template<typename F>
        requires((Invocable<F, const Ts&> && ...) && All_Same<Invoke_Result<F, const Ts&>...>)
    [[nodiscard]] auto match(F&& f) const noexcept {
        return Accessors<With_Lvalue_Ref, true>::apply(spp::forward<F>(f), data_, index_);
    }
...
private:
    ...
    void construct(V&& value) noexcept {
        ...
        new(data_) V{spp::forward<V>(value)};
...
#define INDEX(n)                                                                                   \
    if constexpr(N > n)                                                                            \
        if(index == n) return apply_one<F, Choose<n, Ts...>>(spp::forward<F>(f), data)
        ...
        [[nodiscard]] static auto apply(F&& f, Data* data, u8 index) noexcept {
            ...
            INDEX(2);
            INDEX(1);
            return apply_one<F, Choose<0, Ts...>>(spp::forward<F>(f), data);
        }
...
#undef INDEX
        ...
        template<typename F>
        [[nodiscard]] static auto apply(F&& f, Data* data, u8 index) noexcept {
            return apply_n(spp::forward<F>(f), data, index, Index_Sequence_For<Ts...>{});
        }
    ...
    private:
        ...
        [[nodiscard]] static auto apply_one(F&& f, Data* data) noexcept {
            ...
            return spp::forward<F>(f)(static_cast<Ref<T>>(*launder(reinterpret_cast<Ptr>(data))));
        }
        ...
        [[nodiscard]] static auto apply_n(F&& f, Data* data, u8 index,
            ...
            return table[index](spp::forward<F>(f), data);
        }
    };
...
template<Literal N, typename T>
SPP_NAMED_TEMPLATE_RECORD(Named, SPP_PACK(Named<N, T>)::name, SPP_PACK(N, T), SPP_FIELD(value));

template<typename... Ts>
SPP_TEMPLATE_RECORD(Variant, SPP_PACK(Ts...), SPP_FIELD(data_), SPP_FIELD(index_));
