...
struct Box {
    ...
    {
        data_ = A::template make<T>(spp::move(value));
    }

    template<typename... Args>
        requires Constructable<T, Args...>
    explicit Box(Args&&... args) noexcept {
        data_ = A::template make<T>(spp::forward<Args>(args)...);
    }
    ...
    [[nodiscard]] static Box make(Args&&... args) noexcept {
        ...
        new(data->data()) T{spp::forward<Args>(args)...};
...
SPP_TEMPLATE_RECORD(Box, SPP_PACK(T, P), SPP_FIELD(data_));
