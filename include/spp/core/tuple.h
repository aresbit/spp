...
struct Tuple<T, Ts...> {
    ...
    [[nodiscard]] constexpr auto invoke(F&& f) noexcept -> Invoke_Result<F, T, Ts...> {
        return invoke_(spp::forward<F>(f), Make_Index_Sequence<1 + sizeof...(Ts)>{});
    }
...
template<>
SPP_TEMPLATE_RECORD(Tuple, SPP_PACK());

template<typename F, typename... Ts>
SPP_TEMPLATE_RECORD(Tuple, SPP_PACK(F, Ts...), SPP_FIELD(first), SPP_FIELD(rest));
...
template<typename... Ts>
struct tuple_size<Tuple<Ts...>> : spp::Constant<spp::u64, sizeof...(Ts)> {};

template<size_t Index, typename... Ts>
struct tuple_element<Index, Tuple<Ts...>> {
    using type = decltype(spp::detail::declval<Tuple<Ts...>>().template get<Index>());
};
