...
#define SPP_LOCATION_HASH ::spp::hash_literal(__FILE__, __LINE__)
...
[[nodiscard]] constexpr u64 hash(T&& value) noexcept {
    return Hash::Hash<Decay<T>>::hash(spp::forward<T>(value));
}

template<Hashable T>
[[nodiscard]] constexpr u64 hash_nonzero(T&& value) noexcept {
    return Hash::Hash<Decay<T>>::hash(spp::forward<T>(value)) | 1;
}

template<Hashable... Ts>
[[nodiscard]] constexpr u64 hash(Ts&&... values) noexcept {
    return Hash::squirrel5((hash(spp::forward<Ts>(values)) + ...));
}
