...
#define SPP_PACK(...) __VA_ARGS__
#define SPP_CONCAT(A, B) A##B
#define SPP_OFFSETOF(TYPE, FIELD) __builtin_offsetof(TYPE, FIELD)
#define SPP_CASE(CASE) ::spp::Reflect::Case<__T, __T::CASE, #CASE>
#define SPP_FIELD(FIELD)                                                                           \
    ::spp::Reflect::Field<decltype(__T::FIELD), SPP_OFFSETOF(__T, FIELD), #FIELD>

#define SPP_ENUM(TYPE, DEFAULT, ...)                                                               \
    template<>                                                                                     \
    struct ::spp::Reflect::Refl<TYPE> {                                                            \
        using __T = TYPE;                                                                          \
...
#define SPP_NAMED_ENUM(TYPE, NAME, DEFAULT, ...)                                                   \
    template<>                                                                                     \
    struct ::spp::Reflect::Refl<TYPE> {                                                            \
        using __T = TYPE;                                                                          \
        using underlying = Underlying<TYPE>;                                                       \
...
#define SPP_NAMED_RECORD(TYPE, NAME, ...)                                                          \
    template<>                                                                                     \
    struct ::spp::Reflect::Refl<TYPE> {                                                            \
        using __T = TYPE;                                                                          \
        constexpr static Literal name = NAME;                                                      \
...
#define SPP_TEMPLATE_RECORD(TYPE, ARG_PACK, ...)                                                   \
    struct ::spp::Reflect::Refl<TYPE<ARG_PACK>> {                                                  \
...
#define SPP_NAMED_TEMPLATE_RECORD(TYPE, NAME, ARG_PACK, ...)                                       \
    struct ::spp::Reflect::Refl<TYPE<ARG_PACK>> {                                                  \
        using __T = TYPE<ARG_PACK>;                                                                \
        constexpr static Literal name = NAME;                                                      \
        constexpr static Kind kind = Kind::record_;                                                \
...
concept Apply = requires(F&& f) {
    { spp::forward<F>(f).template apply<X>() } -> Same<void>;
};
...
struct Iter<F, Cons<Head, Tail>> {
    ...
    constexpr static void apply(G&& f) noexcept {
        spp::forward<G>(f).template apply<Head>();
        Iter<F, Tail>::apply(spp::forward<F>(f));
    }
};
...
struct Iter_Case {
    ...
    constexpr void apply() noexcept {
        spp::forward<F>(f)(Case::name, Case::value);
    }
    F&& f;
};
...
constexpr void iterate_enum(F&& f) noexcept {
    Iter<Iter_Case<F>, typename Refl<E>::members>::apply(Iter_Case<F>{spp::forward<F>(f)});
}
...
constexpr void iterate_record(F&& f, R& record) noexcept {
    ...
    Iter<Iter_Field<F, false>, typename Refl<R>::members>::apply(
        Iter_Field<F, false>{spp::forward<F>(f), address});
}
...
constexpr void iterate_record(F&& f, const R& record) noexcept {
    ...
    Iter<Iter_Field<F, true>, typename Refl<R>::members>::apply(
        Iter_Field<F, true>{spp::forward<F>(f), address});
}
...
struct Refl<Kind> {
    ...
    using members = List<SPP_CASE(void_), SPP_CASE(i8_), SPP_CASE(i16_), SPP_CASE(i32_),
                         SPP_CASE(i64_), SPP_CASE(u8_), SPP_CASE(u16_), SPP_CASE(u32_),
                         SPP_CASE(u64_), SPP_CASE(f32_), SPP_CASE(f64_), SPP_CASE(bool_),
                         SPP_CASE(array_), SPP_CASE(pointer_), SPP_CASE(enum_), SPP_CASE(record_)>;
};
