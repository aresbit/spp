...
SPP_RECORD(String_View, SPP_FIELD(data_), SPP_FIELD(length_));

template<Allocator A>
SPP_TEMPLATE_RECORD(String, A, SPP_FIELD(data_), SPP_FIELD(length_), SPP_FIELD(capacity_));
...
struct Hash<String<A>> {
    [[nodiscard]] static u64 hash(const String<A>& string) noexcept {
        u64 h = 0;
        for(u8 c : string) h = hash_combine(h, spp::hash(c));
        return h;
    }
};
...
[[nodiscard]] SPP_FORCE_INLINE spp::String_View operator""_v(const char* c_string,
                                                             size_t length) noexcept {
    return spp::String_View{reinterpret_cast<const spp::u8*>(c_string), length};
}
