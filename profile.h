...
#ifdef SPP_RELEASE_BUILD
constexpr bool DO_PROFILE = false;
#else
constexpr bool DO_PROFILE = true;
#endif

#define SPP_TRACE2(COUNTER) __profile_##COUNTER
#define SPP_TRACE1(NAME, COUNTER) if(::spp::Profile::Scope SPP_TRACE2(COUNTER){String_View{NAME}})

#define Trace(NAME) SPP_TRACE1(NAME, __COUNTER__)
...
struct Profile {
    ...
    struct Scope {
        Scope(Log::Location loc) noexcept {
            Profile::enter(spp::move(loc));
        }
        Scope(String_View name) noexcept {
            Profile::enter(spp::move(name));
        }
        ~Scope() noexcept {
            Profile::exit();
        }
...
SPP_RECORD(Profile::Alloc, SPP_FIELD(name), SPP_FIELD(address), SPP_FIELD(size));
